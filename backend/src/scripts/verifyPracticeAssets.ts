...
async function main(): Promise<void> {
  const questions = await prisma.examVersionQuestion.findMany({ where: { examVersion: { status: 'published' } }, select: { id: true, assets: true } });
  ...
  console.log(JSON.stringify({ publishedQuestions: questions.length, assetReferences: [...usage.values()].reduce((sum, value) => sum + value, 0), duplicateSrcUsage: [...usage.entries()].filter(([, value]) => value > 1).sort(), issues: issues.sort() }, null, 2));
