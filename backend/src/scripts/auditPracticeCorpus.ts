...
async function main(): Promise<void> {
  const [topics, published, draft, publishedVersions, draftVersions, allQuestions] = await Promise.all([
  ...
  const p = toMap(published); const d = toMap(draft); const taxonomy = new Map(topics.map((topic) => [topic.slug, new Set(topic.subtopics.map((subtopic) => subtopic.slug))]));
  ...
  console.log(JSON.stringify({ publishedExamVersions: publishedVersions, draftExamVersions: draftVersions, publishedQuestions: rows.reduce((sum, row) => sum + row.total, 0), draftQuestions: rows.reduce((sum, row) => sum + row.draft.total, 0), typeTotals, coverageLevels: levels, taxonomyAnomalies: [...new Set(anomalies)].sort(), rows }, null, 2));
}
