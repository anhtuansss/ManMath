...
export function ProfileClient() {
  ...
  return <main className="mx-auto max-w-3xl p-6"><p className="workspace-eyebrow">Tài khoản</p><h1 className="workspace-page-title mt-1">Hồ sơ của bạn</h1><section className="mt-6 rounded-xl border border-border bg-surface p-6 shadow-card"><p className="text-xl font-semibold">{user.fullName ?? 'Người dùng ManMath'}</p><p className="workspace-metadata mt-2">{user.email}</p><div className="mt-6 flex flex-wrap gap-3"><Link className="workspace-button-text rounded-lg bg-primary px-4 py-2 text-white" href="/learning">Xem tiến độ</Link><Link className="workspace-button-text rounded-lg border border-border px-4 py-2" href="/history">Lịch sử đề</Link><button className="workspace-button-text rounded-lg border border-border px-4 py-2" onClick={() => { clearAuthToken(); setUser(null); }}>Đăng xuất</button></div></section></main>;
}
